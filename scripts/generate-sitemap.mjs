...
function build() {
  ...
  // 店舗別ページ（データファイルから動的抽出＝追加店舗は自動でsitemap収録）
  for (const f of ['g1-stores.ts', 'the-silk-stores.ts']) {
    const src = fs.readFileSync(new URL(`../data/${f}`, import.meta.url), 'utf8')
    for (const m of src.matchAll(/"?urlSlug"?:\s*"([a-z0-9-]+)"/g)) {
