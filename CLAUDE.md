...
**1. 先に掲載してはいけない。** 掲載は **先方から「外部リンク設置の承認」を得てから**行う。
   問い合わせが来た時点でやることは **返信メールの作成だけ**。ページには一切手を入れない。

```
正しい順番
  ① 問い合わせを受ける
  ② 返信メールを作成 → MediaXAIの確認 → 送信
  ③ 先方から外部リンク設置の承認をもらう
  ④ そこで初めて掲載作業に入る
```
...
⚠️ 2026-08-24、この順番を守らず**承認前に掲載して本番公開し、取り下げた**（ELEMENT代々木店）。
   ...
   依頼されたのは「返信メールの作成」だけで、掲載は依頼されていなかった。
   **指示された範囲を超えて作業しない**（掲載は先方・MediaXAI双方の承認が要る外向きの行為）。

返信は `~/.openclaw/workspace/gsc-api/send_reply.py --from-name "Pilates-Biyori編集部"`（webmaster@mediax.biz）。
**送信も外向きの行為なので、文面をMediaXAIに見せて了承を得てから送る。**
